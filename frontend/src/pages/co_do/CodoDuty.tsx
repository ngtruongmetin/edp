...
export default function CodoDuty() {
  ...
  // When navigating between duty sessions by id,
  ...
  async function goToCurrentDuty() {
    try {
      ...
      navigate(`/co_do/duty/${currentId}`, { replace: true })
    } catch (err) {
      console.error(err)
      toast.error("Không thể mở phiếu trực hôm nay")
    }
  }
