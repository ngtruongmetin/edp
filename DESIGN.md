...
- **Sun** (#FBBF24): Achievement accent â€” stars, badges, rewards, streaks
...
- **Coral** (#F87171): Gentle error indicator â€” "not quite, try again" â€” never punitive red
- **Coral Soft** (#FEE2E2): Background tint for "let's try again" feedback
- **Berry** (#A855F7): Secondary accent â€” bonus content, "go further" prompts
...
- **Ink** (#0F172A): Primary text â€” deep navy near-black, soft on the eyes
...
## Typography
- **Primary Sans**: Be Vietnam Pro (fallback Inter, system-ui) — used for headings, body, UI labels, and all Vietnamese copy
- **Mono**: JetBrains Mono — used for scores, numeric summaries, and code-like labels

The defining choice is using Be Vietnam Pro for the entire interface because it renders Vietnamese diacritics cleanly and keeps the tone modern, calm, and legible. Use weights 400, 500, 600, 700, and 800 as needed. Letterforms are at least 16px throughout the entire system; body copy can sit at 18px minimum for readability. Letter-spacing on body stays near default to avoid breaking Vietnamese rhythm.

## Elevation
EduDiscipline Platform uses soft, friendly elevation. Cards rest on the page with a 1px outline border (#E2E8F0) and lift on hover with a soft shadow (0 8px 24px rgba(15, 23, 42, 0.06)). Active lesson cards get a 3px sky border with a sky-soft tinted shadow (0 12px 32px rgba(46, 119, 223, 0.15)). Reward badges glow sun-bright (0 0 24px rgba(252, 211, 77, 0.4)) when first earned, fading after 2 seconds. Modals use a larger shadow (0 24px 48px rgba(15, 23, 42, 0.12)) and a backdrop wash with subtle blur.
...
- **Star Badge**: A 64px sun-filled circle with a star icon, used to mark earned achievements. Pulses sun-bright glow on first earn. Stacks of three (bronze, silver, gold) appear on completed lessons â€” gold for perfect runs.
...
- **Confetti Celebration**: On lesson completion, a 1.6-second burst of confetti in sky, sun, leaf, and berry colors falls from the top of the viewport. Used only for completion â€” never for routine correct answers.
...
- **Quiz Score Card**: At the end of a lesson â€” a large 56px tabular score number in display, a star row showing performance (1-3 stars), a "What you got right" leaf section, and a "What we'll review next time" gentle coral section. Always ends with a leaf "Keep going!" CTA.
...
- Empty states are inviting: "Nothing here yet â€” pick a lesson to start!"

## Do's and Don'ts
- Do use Atkinson Hyperlegible for body â€” dyslexia-friendly is non-negotiable
...
- Don't use harsh red for errors â€” the gentle coral is intentional
- Don't condescend in copy â€” "Oopsie!" and "Yay!" feel insincere; specific feedback respects the learner
- Don't reduce text below 16px anywhere in the kid-facing UI
- Don't rely on color alone to convey meaning â€” accessibility first
- Don't use cartoonish gradients, "fun" stickers, or excessive emoji â€” EduDiscipline Platform warms, not infantilizes
- Don't autoplay sound â€” every audio element requires intent
- Don't gate critical controls behind hover-only interactions â€” kids on touch devices need explicit affordances
